class Level1 extends Phaser.Scene
{
	...
	setup_physics()
	{
		...
		this.physics.add.overlap(this.player, this.group_deadly3, this.game_over,null, this);
	...
	setup_camera()
	{
		...
		this.cameras.main.setBackgroundColor('rgb(35,64,153)');
	}
	...
	create_object()
	{
		...
		this.setup_object(this.group_falling);
		
		var deadly1_image = new Object();
		deadly1_image.key = 'itemMap';
		deadly1_image.frame = 2;
		this.group_deadly1 = this.map.createFromObjects('items', 'spike-up',deadly1_image);
		this.setup_object(this.group_deadly1);
		...
		this.setup_object(this.group_deadly2);
		
		var deadly3_image = new Object();
		deadly3_image.key = 'itemMap';
		deadly3_image.frame =7;
		this.group_deadly3 = this.map.createFromObjects('items', 'water', deadly3_image);
		this.setup_object(this.group_deadly3);
		
		var collectables = new Object();
		collectables.key = 'itemMap';
		collectables.frame = 8
		this.group_coins =this.map.createFromObjects('items', 'lol',collectables);
		this.setup_object(this.group_coins);
		
		var exit = new Object();
		exit.key = 'itemMap';
		exit.frame =3;
		this.goal=this.map.createFromObjects('items','exit',exit)
		this.setup_object(this.goal);
		
	}
	
	setup_object(obj_group)
	{
